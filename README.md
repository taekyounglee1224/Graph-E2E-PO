...
| # | Question | Comparison |
|---|---|---|
| **RQ1** | Does the E2E GNN pipeline outperform classical sample-covariance GMV? | `Graph-L1` vs `SampleGMV` |
| **RQ2** | Does incorporating a return signal in the loss improve risk-adjusted performance? | `Graph-L1` vs `Graph-L2` |
| **RQ3** | Does the graph structure provide useful inductive bias for covariance learning? | `Graph-L1` vs `MLP-L1` |
...
| `MLP-L1` | MLP | $\mathcal{L}_1$ | E2E without graph — RQ3 ablation |
...
$\hat{\Sigma}_\text{sample}$ is the sample covariance of the lookback window — fixed during forward pass to prevent the model from gaming its own evaluation.
...
└── results/              # Generated after running experiment.ipynb (Section 9)
    ...
    ├── ttest_results.csv        # RQ × metric × t-stat / p-value
    ├── stat_distributions.png   # Violin plots
    └── pvalue_heatmap.png       # p-value heatmap
```
...
- **Section 9** : N-seed loop (default N=10) → saves `results/experiment_results.csv`
...
- Paired t-test results for RQ1 / RQ2 / RQ3
- `results/ttest_results.csv`
- `results/stat_distributions.png` — metric distributions across seeds
- `results/pvalue_heatmap.png` — p-value heatmap by RQ × metric
...
| `d_hidden` | 32 | GCN hidden dimension |
